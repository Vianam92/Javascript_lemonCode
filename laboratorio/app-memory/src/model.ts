...
const crearCartaInicial = (idFoto: number, imagen: string): Card => ({
  idFoto,
  imagen,
  estaVuelta: false,
  encontrada: false,
...
const crearColeccionDeCartasInicial = (infoCards: InfoCards[]): Card[] => {
  return infoCards
    .map((card: InfoCards) => crearCartaInicial(card.idFoto, card.imagen))
    .reduce(
      (acc: Card[], cur: Card) => [...acc, cur, cur],
      []
    );
};

export let cartas: Card[] = crearColeccionDeCartasInicial(infoCards);
